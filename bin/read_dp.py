...
chrom_lengths = {}
with gzip.open(SAM_PATH + "reference_genome/GRCh38_latest_genomic.fna.gz", "rt") as handle:
...
for rec in ParseSam(SAM_PATH + "alignments/female_" + str(args.depth) + "_align.sam"):
...
for ids in these_ids:
        avg_list.append(avg_cov["NC_000023.11"]/avg_cov[ids])
        
plt.figure(0)    
plt.scatter(these_ids, avg_list)
plt.ylim([0,2])
plt.title("X:Autosomal Coverage Ratio - Female (Read Depth:" + str(args.depth) + ")")
plt.ylabel('Normalized Coverage')
plt.xlabel('Chromosome ID')
plt.xticks(rotation = -90)
plt.savefig('../results/figures/x_to_autosome_female_' + args.depth + '_zonkey.png', bbox_inches='tight')
...
for rec in ParseSam(SAM_PATH + "alignments/male_" + str(args.depth) + "_align.sam"):
    if not_in_list(["SECONDARY", "UNMAP", "MUNMAP", "SUPPLEMENTARY"], decompose_sam_flag(rec.flag)) == True:
        if "NW" not in rec.rnam and "NT" not in rec.rnam:
            if rec.rnam in hit_counts:
                hit_counts[rec.rnam] += 1
...
plt.scatter(these_ids, avg_list)
plt.ylim([0,2])
plt.title("X:Autosomal Coverage Ratio - Male (Read Depth:" + str(args.depth) + ")")
plt.ylabel('Normalized Coverage')
plt.xlabel('Chromosome ID')
plt.xticks(rotation = -90)
plt.savefig('../results/figures/x_to_autosome_male_' + str(args.depth) + '_zonkey.png', bbox_inches='tight')

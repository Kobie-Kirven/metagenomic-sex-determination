...
for rec in ParseSam(SAM_PATH + "alignments/female_10000_align.sam"):
...
plt.savefig('../results/figures/x_to_autosome_female_10000_zonkey.png', bbox_inches='tight')
...
for rec in ParseSam(SAM_PATH + "alignments/male_10000_align.sam"):
    if not_in_list(["SECONDARY", "UNMAP", "MUNMAP", "SUPPLEMENTARY"], decompose_sam_flag(rec.flag)) == True:
        if "NW" not in rec.rnam and "NT" not in rec.rnam:
            if rec.rnam in hit_counts:
                hit_counts[rec.rnam] += 1
...
plt.xlabel('Chromosome ID')
plt.xticks(rotation = -90)
plt.savefig('../results/figures/x_to_autosome_male_10000_zonkey.png', bbox_inches='tight')
